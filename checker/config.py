...
DEFAULT_CONFIG: dict = {
    ...
    # По умолчанию окно видимое: сайт закрыт защитой Cloudflare, которая
    # иногда просит подтвердить, что вы не робот, — сделать это можно
    # только руками, в настоящем окне.
    "headless": False,
    ...
    "browser_path": "",
    # Сколько секунд ждать, пока человек пройдёт проверку «я не робот».
    "challenge_wait_seconds": 180,
    # Подключаться к уже запущенному Chrome вместо своего браузера.
    # Пусто — обычный режим. "http://127.0.0.1:9222" — подключиться к Chrome,
    # запущенному файлом chrome-debug.bat. В своём браузере вы проходите
    # проверку «я не робот» как обычный человек, и программа читает
    # страницу в уже подтверждённой сессии.
    "cdp_url": "",
    # Где хранить профиль браузера. Пусто — папка browser-profile рядом
    # с программой. В профиле лежит cookie пройденной проверки Cloudflare,
    # поэтому подтверждать «я не робот» приходится редко.
    "profile_dir": "",
    "sites": [
        {
